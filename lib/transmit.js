...
async function transmit({ invoiceNumber, entityId }) {
  ...
  try {
    ...
    // audit log only — the caller (server.js) decides what, if anything,
    // of this is safe to send back to the browser.
    if (!['NO_MATCH', 'AMBIGUOUS_MATCH'].includes(err.code)) {
      await appendLog(
        `FAIL entity=${entity.id} invoice=${invoiceNumber} token=${searchToken} reason=${err.code || 'error'} detail=${err.message}`
      );
    }
    throw err;
  }
